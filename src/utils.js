...
const emailMaker = (firstName, lastName) => {
  const email = `${firstName}${lastName}@gmail.com`
  return email
}

const staffEmailMaker = (firstName, lastName) => {
  const email = `${firstName}${lastName}@tandartspraktijkbvt.nl`
  return email
}

const generateRandomPatient = (index) => {
  const first = getRandomFirst()
  const last = getRandomLast()
  const email = emailMaker(first, last)

  return {
    firstName: first,
    lastName: last,
    phoneNr: getRandomPhone(),
    email: email,
    birthYear: getRandomBirth(),
    healthy: true,
    id: index
  }
}

const generateRandomStaff = (index) => {
  const first = getRandomFirst()
  const last = getRandomLast()
  const email = staffEmailMaker(first, last)
...
const generateRandomPatients = num =>
  Array(num)
    .fill(0)
    .map((_, index) => generateRandomPatient(index));

const generateRandomStaffs = num =>
  Array(num)
    .fill(0)
    .map((_, index) => generateRandomStaff(index));

const patients = generateRandomPatients(50)
const dentists = generateRandomStaffs(4)
const assistants = generateRandomStaffs(2)

const getRandomDentist = () => {
  let dentist = dentists[Math.floor(Math.random() * 4)]
  return dentist
}

const getRandomAssistant = () => {
  let assistant = assistants[Math.floor(Math.random() * 2)]
  return assistant
}

const getRandomPatient = () => {
  let patient = patients[Math.floor(Math.random() * 50)]
  return patient
}
...
const generateRandomAppointments = num =>
  Array(num)
    .fill(0)
    .map((_, index) => generateRandomAppointment(index));

const appointments = generateRandomAppointments(150).sort((a, b) => a.time - b.time)

const variables = { appointments, patients, dentists, assistants }
